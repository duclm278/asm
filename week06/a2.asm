...
after_max:	lw		$t0, 0($a1)	#load last element into $t0
		...
		j		print
after_print:	j		sort		#repeat sort for smaller list
done:		j		after_sort
#------------------------------------------------------------------------
#Procedure max
...
print:		add		$s7, $v0, $zero		#Save
		...
		la		$s1, Aend
		addi		$s1, $s1, -4		#$s1 = Address(A[n-1])

loop2:		blt		$s1, $s0, endprint	#exit if Aned < A[i]

		li		$v0, 1			#service 01: print integer
		lw		$a0, 0($s0)		#value of A[i]
		syscall
		li		$v0, 11
		li		$a0, ' '			#print space
		syscall
		addi		$s0, $s0, 4		#Move to A[i+1]
		j		loop2
